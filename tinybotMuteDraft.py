...
class TinychatBot(pinylib.TinychatRTCClient):
    ...
    def cmd_handler(self, msg):
        ...
        if cmd == prefix + 'lamer':  # dirty 27 minutes hack code /// requested guest squelch
            _user = self.users.search_by_nick(cmd_arg)
            if _user is not None:
                if cmd_arg in self.lamers:
                    ...
                    _user.user_level = 8
                    self.lamers.append(cmd_arg)
                    if _user.is_broadcasting:
                        self.send_close_user_msg(_user.id)
                        _user.is_broadcasting = False
            else:
                self.handle_msg('FAIL! case sensitive.')
            self.handle_msg('%s is a lamer! Squelching lamer.' % cmd_arg)
    ...
    def message_handler(self, msg, uid):

        _user = self.users.search(uid)

        prefix = pinylib.CONFIG.B_PREFIX

        if pinylib.CONFIG.B_MUTE_CAMBAN:
            if _user is not None:
                if _user.user_level == 8 or _user.nick in self.lamers:
                    self.score = 10
