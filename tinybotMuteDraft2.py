...
class TinychatBot(pinylib.TinychatRTCClient):
    ...
    def cmd_handler(self, msg):
        ...
        if cmd == prefix + 'llama':  # dirty 300 minutes hack code /// requested guest squelch
            _user = self.users.search_by_nick(cmd_arg)
            if _user is not None:
                if cmd_arg in self.lamers:
                    self.lamers.remove(cmd_arg)
                    _user.user_level = 7
                    self.handle_msg('%s is no longer considered a lamer.' % cmd_arg)
                    return
                else:
                    _user.user_level = 8
                    self.cambans.append(cmd_arg)
                ...
                self.handle_msg('FAIL! case-sensitive.')
            self.handle_msg('Squelching lamer: %s' % cmd_arg)
    ...
    def on_lamer(self, uid, msg):
        ...
        if _user or self.active.user.last_msg =
                if self_user.user_level == 8 or _user in self.lamers:




    def on_lamer2(self, uid, msg):
        _user = self.users.search(uid)
        if pinylib.CONFIG.B_MUTE_LAMERS:
            if _user is not None:
                if _user.user_level == 8 or _user.nick in self.cambans:
                    self.score = 10
                    self.send_close_user_msg(_user.id)
            ...
            self.send_kick_user_msg(_user.id)
            _user.is_lame = False
